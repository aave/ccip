...
contract GHOTokenPoolEthereumBridgeLimitSetup is BaseTest {
  ...
  uint256[] internal chainsList;
  mapping(uint256 => address) internal pools; // chainId => bridgeTokenPool
  mapping(uint256 => address) internal tokens; // chainId => ghoToken
  mapping(uint256 => uint256) internal bucketCapacities; // chainId => bucketCapacities
  mapping(uint256 => uint256) internal bucketLevels; // chainId => bucketLevels
  mapping(uint256 => uint256) internal liquidity; // chainId => liquidity
  uint256 internal remoteLiquidity;
  uint256 internal bridged;
